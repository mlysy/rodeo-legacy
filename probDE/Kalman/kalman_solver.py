...
def kalman_solver(fun, tmin, tmax, n_eval, mu, sigma, roots, w, init, draws=1):
    ...
    Args:
        ...
        mu (ndarray(p)): Mean vector of the CAR(p) process.
        sigma (ndarray(1)): Scale parameter of the CAR(p) process.
        roots (ndarray(p)): Roots for the CAR(p) process.
        w (ndarray(q+1)): Corresponds to the :math:`w` vector in the ODE equation.
        init (ndarray(q) or ndarray(p)): Initial values. Either :math:`a`, or :math:`X_L = (a, y_L)`.
            In case of the former, y_L is drawn from stationary distribution condional on :math:`x_L = a`.
        draws (int): Number of draws to simulate from the solver.
    
    Returns:
        (tuple):
        - **XSample** (ndarray(n_eval, p)): Sample solution at time t given observations from times [0...N] 
          for :math:`t = 0,1/N,\ldots,1`.
        - **meanX** (ndarray(n_eval, p)): Posterior mean of the solution process :math:`y_n` at times :math:`t = 0,1/N,\ldots,1`.
        - **varX** (ndarray(n_eval, p, p)): Posterior variance of the solution process at times :math:`t = 0,1/N,\ldots,1`.
