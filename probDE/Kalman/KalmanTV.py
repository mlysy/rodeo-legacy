...
class KalmanTV(object):
    ...
    def predict(self, 
        ...
        var_state_pred = np.linalg.multi_dot([wgt_state, var_state_past, wgt_state.T]) + var_state
    ...
    def update(self,
        ...
        var_meas_meas_pred = np.linalg.multi_dot([wgt_meas, var_state_pred, wgt_meas.T]) + var_meas
        var_state_meas_pred = var_state_pred.dot(wgt_meas.T)
        var_state_temp = solveV(var_meas_meas_pred, var_state_meas_pred.T).T
        mu_state_filt = mu_state_pred + var_state_temp.dot(x_meas - mu_meas_pred)
        var_state_filt = var_state_pred - var_state_temp.dot(var_meas_state_pred)
    ...
    def smooth_mv(self,
        ...
        mu_state_smooth = mu_state_filt + var_state_temp_tilde.dot(mu_state_next - mu_state_pred)
        var_state_smooth = var_state_filt + np.linalg.multi_dot([var_state_temp_tilde, (var_state_next - var_state_pred), var_state_temp_tilde.T])
    ...
    def smooth_sim(self,
        ...
        x_state_smooth = np.random.multivariate_normal(mu_state_sim, var_state_sim, tol=1e-6)
