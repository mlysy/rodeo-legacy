...
def kalman_ode_offline_py(fun, x0_state, tmin, tmax, n_eval, wgt_state, mu_state, var_state, wgt_meas, z_state_sim,
                     x_meass, smooth_mv=True, smooth_sim=False, filter_only=False, offline=True):
    ...
    var_meass = np.zeros((n_dim_meas, n_dim_meas, n_steps))
    mu_state_filts = np.zeros((n_dim_state, n_steps))
    var_state_filts = np.zeros((n_dim_state, n_dim_state, n_steps))
    mu_state_preds = np.zeros((n_dim_state, n_steps))
    var_state_preds = np.zeros((n_dim_state, n_dim_state, n_steps))
    mu_state_smooths = np.zeros((n_dim_state, n_steps))
    var_state_smooths = np.zeros((n_dim_state, n_dim_state, n_steps))
    x_state_smooths = np.zeros((n_dim_state, n_steps))
        ...
        if offline:
            mu_state_preds[:, t+1], var_state_preds[:, :, t+1], \
                mu_state_filts[:, t+1], var_state_filts[:, :, t+1] = (
                    KFS.filter(mu_state_past=mu_state_filts[:, t],
