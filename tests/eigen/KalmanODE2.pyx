...
cdef class KalmanODE:
    ...
    cpdef _solve_filter(self, double[::1] x0, object theta=None):
        ...
        for t in range(self.n_eval):
            # kalman filter:
            self.ktv.predict(& self.mu_state_pred[:, t+1][0],
                             & self.var_state_pred[:, :, t+1][0,0],
                             & self.mu_state_filt[:, t][0],
                             & self.var_state_filt[:, :, t][0, 0],
                             & self._mu_state[0],
                             & self._wgt_state[0, 0],
                             & self._var_state[0, 0])
    ...
    cpdef solve(self, double[::1] x0, double[::1, :] W=None, object theta=None):
        ...
        self.ktv.state_sim(& x_state_smooth[:, self.n_eval][0],
                           & mu_state_smooth[:, self.n_eval][0],
                           & var_state_smooth[:, :, self.n_eval][0, 0],
                           & self._z_state[:, self.n_eval][0])
        # backward pass
        cdef int t
        for t in range(self.n_eval-1, 0, -1):
            self.ktv.smooth(& x_state_smooth[:, t][0],
                            & mu_state_smooth[:, t][0],
                            & var_state_smooth[:, :, t][0,0],
                            & x_state_smooth[:, t+1][0],
                            & mu_state_smooth[:, t+1][0],
                            & var_state_smooth[:, :, t+1][0,0],
                            & self.mu_state_filt[:, t][0],
                            & self.var_state_filt[:, :, t][0,0],
                            & self.mu_state_pred[:, t+1][0],
                            & self.var_state_pred[:, :, t+1][0,0],
                            & self._wgt_state[0,0],
                            & self._z_state[:, t+self.n_steps][0])
    ...
    cpdef solve_sim(self, double[::1] x0, double[::1, :] W=None, object theta=None):
        ...
        self.ktv.state_sim(& x_state_smooth[:, self.n_eval][0],
                           ...
                           & self.var_state_filt[:, :, self.n_eval][0, 0],
                           & self._z_state[:, self.n_eval][0])
        # backward pass
        cdef int t
        for t in range(self.n_eval-1, 0, -1):
            self.ktv.smooth_sim(& x_state_smooth[:, t][0],
                                & x_state_smooth[:, t+1][0],
                                & self.mu_state_filt[:, t][0],
                                & self.var_state_filt[:, :, t][0,0],
                                & self.mu_state_pred[:, t+1][0],
    ...
    cpdef solve_mv(self, double[::1] x0, double[::1, :] W=None, object theta=None):
        ...
        cdef int t
        for t in range(self.n_eval-1, 0, -1):
            self.ktv.smooth_mv(& mu_state_smooth[:, t][0],
                               & var_state_smooth[:, :, t][0,0],
                               & mu_state_smooth[:, t+1][0],
                               & var_state_smooth[:, :, t+1][0,0],
                               & self.mu_state_filt[:, t][0],
