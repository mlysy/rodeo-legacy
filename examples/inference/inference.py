...
class inference:
    ...
    def kalman_nlpost(self, phi, Y_t, x0, step_size, phi_mean, phi_sd, kalman_solve, loglik, *args):
        ...
        X_t = kalman_solve(data_tseq, ode_tseq, x0, theta)
    ...
    def euler_nlpost(self, phi, Y_t, x0, step_size, phi_mean, phi_sd, euler_solve, loglik, *args):
        r"Compute the negative loglikihood of :math:`Y_t` using the Euler method."
        theta = np.exp(phi)
        X_t = euler_solve(x0, step_size, theta)
        lp = loglik(Y_t, X_t, *args)
        lp += self.loglike(phi, phi_mean, phi_sd)
        return -lp
    
    def phi_fit(self, Y_t, x0, step_size, theta_true, phi_sd, obj_fun, solve, loglik, *args):
        ...
        opt_res = sp.optimize.minimize(obj_fun, phi_init,
                                    args=(Y_t, x0, step_size, phi_mean, phi_sd, solve, loglik, *args),
                                    method='Nelder-Mead')
        phi_hat = opt_res.x
        hes = nd.Hessian(obj_fun)
        phi_fisher = hes(phi_hat, Y_t, x0, step_size, phi_mean, phi_sd, solve, loglik, *args)
