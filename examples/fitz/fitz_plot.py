...
class fitz_plot:
    ...
    def simulate(self, x0, theta_true, gamma):
        "Simulate observed data for inference"
    ...
    def ode_nlpost(self, phi, Y_t, x0, h, phi_mean, phi_sd, gamma):
        "Compute the negative loglikihood of Y_t using the odeint solver."
        theta =  np.exp(phi)
        tseq = np.linspace(self.tmin, self.tmax, self.tmax-self.tmin+1)
    ...
    def theta_plot_single(self, Theta, theta_true, h):
        "Plot the distribution of theta using the Kalman solver or the Euler approximation."
